import fs from 'fs';
import { parseHtmlXls, mergeTrades, calculateSummaryStats, getTickerSummaries } from './src/utils/excelParser';
...
console.log('\n===================================================');
console.log('--- MERGED & DEDUPLICATED RESULTS ---');
console.log(`Total Files: ${results.length}`);
console.log(`Total Merged Unique Trades: ${merged.mergedTrades.length}`);
...
console.log(`Total Fee & Tax: ${stats.totalFeeAndTax.toLocaleString('vi-VN')} VND`);
console.log(`Overall ROI: ${stats.overallRoiPercent.toFixed(2)}%`);
console.log(`Win Rate: ${stats.winRatePercent.toFixed(2)}% (${stats.winningTrades} Wins / ${stats.losingTrades} Losses)`);
...
console.log('\n--- TICKER SUMMARIES (TOP 5 WINNERS & LOSERS) ---');
...
tickerSummaries.slice(-5).reverse().forEach(t => {
  console.log(`  ${t.ticker} (${t.assetType}): ${t.netProfit.toLocaleString('vi-VN')} VND (${t.tradeCount} trades)`);
});
